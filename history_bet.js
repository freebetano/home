// ==========================================
// MÓDULO: OUTROS_MERCADOS.JS (POLLING ATIVO A CADA 5s)
// ==========================================
...
const MercadoManager = {
    ...
    iniciarPolling(jogoId, containerId = 'containerOutrosMercados') {
        ...
        // 1ª busca imediata (monta a tela)
        this.requisitarAtualizar(jogoId, containerId, false);

        // Loop contínuo a cada 5 segundos (5000ms)
    ...
    async requisitarAtualizar(jogoId, containerId, isSilencioso) {
        ...
        try {
            ...
            const op1 = dadosJogo.opponent1?.fullName || dadosJogo.homeName || "Casa";
            const op2 = dadosJogo.opponent2?.fullName || dadosJogo.awayName || "Fora";
            ...
            // ====================================================
            // SEGUNDO PLANO (A CADA 5s): ATUALIZA APENAS O VALOR DAS ODDS
            // ====================================================
                        ...
                        grupo.events.forEach((subLista, sIndex) => {
                            subLista.forEach((ev, eIndex) => {
                                ...
                                if (oddSpan && oddSpan.textContent !== novaOdd) {
                                    ...
                                    // Pisca em laranja indicando a alteração da odd
                                    oddSpan.style.color = 'var(--brand-orange)';
                                    setTimeout(() => {
                                        if (oddSpan) oddSpan.style.color = 'var(--brand-teal)';
                                    }, 1200);

                                    // Se estiver no boletim, atualiza a odd dele em tempo real
                ...
                if (Array.isArray(grupo.events)) {
                    grupo.events.forEach((subLista, sIndex) => {
                        subLista.forEach((ev, eIndex) => {
                            ...
                            const onclickMarket = isDesativado ? '' : `onclick="alternarOdd('${op1} vs ${op2}', '${tituloGrupo}: ${selecaoParaBoleto}', '${oddCf}', '${jogoId}', '${btnIdMarket}', this, ${gIndex}, ${sIndex}, ${eIndex})"`;
